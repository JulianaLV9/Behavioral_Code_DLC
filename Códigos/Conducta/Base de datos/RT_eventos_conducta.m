...
%% Sacar los eventos 
    ...
    nombre      = extractBetween(archivo,'B6\', '_RTO_');
...
aligned_values(indices == 1) = original_values(indices == 1);
...
Go_Cue = senal_go(inicioSenalGo == 1);
Barra          = find(barra == 1);
Recompensa     = find(correcto == 1);
Intentos       = find(intento ==1);

tareaInfo = table(Inicio_Ensayos,Fin_Ensayos);

tareaInfo(:,1) = ventanasEnsayos;
